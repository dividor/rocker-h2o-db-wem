#     
# Main rocker image, extended for H2O.AI, rJava, Microsoft SQL server 11 ODBC, text2vec, word2vector,
...
# SQL server horribleness - 11 ODBC. Hopefully we can get a standard deployment when rocker update
# their Linux version.
RUN cd
...
RUN ln -s /usr/lib/x86_64-linux-gnu/libodbcinst.so.2.0.0 /usr/lib/x86_64-linux-gnu/libodbcinst.so.1 ;
RUN ln -s /lib/x86_64-linux-gnu/libodbc.so.2.0.0 /usr/lib/x86_64-linux-gnu/libodbc.so.1;
